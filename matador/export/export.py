...
def doc2param(doc, *args, **kwargs):
    ...
    Keyword arguments:
        ...
        mag_spin (int): enforce breaking of spin symmetry to user supplied number.
    ...
    if doc.get('spin_polarized'):
        ...
        elif mag_spin is not None:
            total_spin = mag_spin
        elif spin:
            total_spin = 5

        if total_spin is not None:
            flines.append("{0:30}: {1}\n".format('spin', total_spin))
    elif mag_spin is not None or spin: 
        #if you've set a spin calc up but not added the spin_polarized keyword then write that in along with the spin state
        ...
        if total_spin is not None:
            flines.append("{0:30}: {1}\n".format('spin', total_spin))
        flines.append("spin_polarized : true")
        ...
        if 'atomic_init_spins' in doc:
            for ind, atom in enumerate(zip(doc['atom_types'], doc['positions_frac'])):
                if atom[0] in doc['atomic_init_spins']:
                    flines.append("{0:8s} {1[0]: 15f} {1[1]: 15f} {1[2]: 15f} SPIN={2:}".format(
                        atom[0], atom[1], doc['atomic_init_spins'][atom[0]]))
                else:
                    flines.append("{0:8s} {1[0]: 15f} {1[1]: 15f} {1[2]: 15f}".format(
                        atom[0], atom[1]))
        else:
            for ind, atom in enumerate(zip(doc['atom_types'], doc['positions_frac'])):
                if ind == 0 and spin:
                    # if spin is True, break spin symmetry on first atom a la run.pl
                    flines.append("{0:8s} {1[0]: 15f} {1[1]: 15f} {1[2]: 15f}\tSPIN=5".format(atom[0], atom[1]))
                elif ind == 0 and mag_spin is not None:
                    # if spin is True, break spin symmetry on first atom a la run.pl
                    flines.append("{0:8s} {1[0]: 15f} {1[1]: 15f} {1[2]: 15f}\tSPIN={2:}".format(atom[0], atom[1],mag_spin))
                else:
                    flines.append("{0:8s} {1[0]: 15f} {1[1]: 15f} {1[2]: 15f}".format(atom[0], atom[1]))
