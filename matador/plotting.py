...
def plot_spectral(seed, bandstructure=True, dos=False, cell=False, **kwargs):
    """ Plot bandstructure and optional DOS from <seed>.bands and
    <seed>.adaptive.dat file.

    Input:

        | seed: str, filename of bands file

    Args:

        | bandstructure : bool, whether to plot bandstructure
        | dos           : bool, whether to plot density of states
    ...
    if bandstructure and not dos:
        ...
        ax_bs = ax_grid[0]
        ax_dos = ax_grid[1]
    elif not bandstructure and dos:
            ...
            for ns in range(bs['num_spins']):
                for nb in range(bs['num_bands']):
                    if np.max(bs['eigenvalues_k_s'][ns][nb][branch]) < 0:
                        ...
                        colour = 'black'
                    ax_bs.plot(path[(np.asarray(branch)-branch_ind).tolist()], bs['eigenvalues_k_s'][ns][nb][branch], c=colour, lw=1, marker=None)
                                    ...
                                    if branch_ind < len(bs['kpoint_branches'])-1:
                                        ...
                                        for new_label, new_point in path_labels.items():
                                            ...
                                            if np.allclose(new_point, next_point):
                                                ...
                                                shear_planes.append(path[ind-branch_ind])
                                label = '${}$'.format(label)
    ...
    if dos:
        ...
        if bandstructure:
            ...
            ax_dos.plot(energies, dos, lw=1, c='k')
