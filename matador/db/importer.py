...
class Spatula:
    ...
    def __init__(self, *args, settings=None):
        ...
        default_collection = recursive_get(
            self.settings, ["mongo", "default_collection"]
        )
        try:
                ...
                if default_collection_file_path is not None:
                    if not os.getcwd().startswith(
                        ...
                        raise RuntimeError(
                            ...
                            "current directory, import must be called from {}".format(
                                default_collection_file_path
                            )
                        )
        else:
        ...
        if not self.scan:
            ...
            if errors == 1:
                self.log.warning(
                    "There is 1 error to view in {}".format(self.logfile.name)
                )
            elif errors == 0:
                self.log.warning(
                    "There are no errors to view in {}".format(self.logfile.name)
                )
            elif errors > 1:
                self.log.warning(
                    "There are {} errors to view in {}".format(
                        ...
                        if elem[0] not in struct["species_pot"]:
                            ...
                            if "OTF" in struct["species_pot"][elem[0]].upper():
                                ...
                                failed_checks.append(
                                    "pspot not fully specified for {}".format(elem[0])
                                )
                    struct["species_pot"] = {
                        species: struct["species_pot"][species] for species in specified
                    }
            ...
            if self.args.get("prototype"):
                ...
                if file_lists[root]["res_count"] > 0:
                    if (
                        ...
                        and file_lists[root]["cell_count"]
                        <= file_lists[root]["res_count"]
                    ):
                        style = "airss"
                if style == "airss":
    ...
    def _scrape_multi_file_results(self, file_lists, root):
        ...
        if file_lists[root]["param_count"] == 1:
            ...
            self.log.warning(
                "Multiple param files found: {}".format(file_lists[root]["param"])
            )
            multi = True
        if file_lists[root]["cell_count"] == 1:
            cell_dict, success = cell2dict(
                file_lists[root]["cell"][0],
                db=True,
                debug=self.debug,
                noglob=True,
                verbosity=self.verbosity,
            ...
            multi = True
            self.log.warning(
                "Multiple param files found: {}".format(file_lists[root]["cell"])
            )
        if multi:
            found_multi = False
            for param_name in file_lists[root]["param"]:
                for cell_name in file_lists[root]["cell"]:
                    if param_name.split(".")[0] in cell_name:
                        ...
                        param_dict, success = param2dict(
                            param_name, debug=self.debug, verbosity=self.verbosity
                        )
                        param = success
                        if not success:
                            self.logfile.write(param_dict)
                        ...
                        if success:
                            found_multi = True
                            self.log.info(
                                "Found matching cell and param files: {}".format(
                                    param_name
                                )
                            )
                            break
            ...
            if not found_multi:
                self.log.warning(
                    "Unable to find matching cell and param files for {}".format(root)
                )
        ...
        for _, _file in enumerate(
            loading_bar(file_lists[root]["res"], verbosity=self.verbosity)
        ):
            ...
            # otherwise, scrape res file
                ...
                struct_dict, success = res2dict(
                    _file, verbosity=self.verbosity, noglob=True
                )
            ...
            if not success:
                ...
                try:
                    ...
                    if (
                        "lattice_cart" not in final_struct
                        and "lattice_abc" not in final_struct
                    ):
                        msg = "! {} missing lattice".format(_file)
                        self.logfile.write(msg)

                    if (
                        "kpoints_mp_spacing" not in final_struct
                        and "kpoints_mp_grid" in final_struct
                    ):
                    ...
                    self.log.error(
                        "Unexpected error for {}, {}".format(_file, final_struct)
                    )
                    raise exc
    ...
    def _scrape_single_file_structures(self, file_lists, root):
        ...
        for _, file in enumerate(
            loading_bar(file_lists[root]["castep"], verbosity=self.verbosity)
        ):
            castep_dict, success = castep2dict(
                file, debug=False, verbosity=self.verbosity
            )
            if not success:
    ...
    def _scrape_prototypes(self, file_lists, root):
        ...
        for _, file in enumerate(file_lists[root]["res"]):
            res_dict, success = res2dict(
                file, db=False, verbosity=self.verbosity, noglob=True
            )
            if not success:
    ...
    def _scan_dir(self):
        ...
        self.log.info(
            "Scanning {} for CASTEP/AIRSS output files... ".format(topdir_string)
        )
        for root, _, files in os.walk(topdir, followlinks=True, topdown=True):
            for pattern in self.exclude_patterns:
                if pattern in root:
                    self.log.debug(
                        "Skipping directory {} as it matched exclude pattern {}".format(
                            root, pattern
                        )
                    )
            ...
            for _file in files:
                ...
                if _file.endswith(".res"):
                    ...
                    _file.endswith(".castep")
                    or _file.endswith(".history")
                    or _file.endswith(".history.gz")
                ):
        ...
        for root in file_lists:
            cached_list = [
                ...
                for entry in file_lists[root]["castep"]
            ]
            duplicates = []
            for ind, file in enumerate(file_lists[root]["castep"]):
                if (
                    not file.endswith(".castep")
                    and cached_list.count(file.split("/")[-1].split(".")[0]) > 1
                ):
                    duplicates.append(ind)
                    file_lists[root]["castep_count"] -= 1
            file_lists[root]["castep"] = [
                entry
                for ind, entry in enumerate(file_lists[root]["castep"])
    ...
    def _scan_dupes(self, file_lists):
        ...
        for _, root in enumerate(file_lists):
            ...
            for structure_type in types:
                if self.recent_only:
                    try:
                        new_file_lists[root][structure_type].sort(
                            key=lambda x: os.stat(x).st_ctime
                        )
                    except Exception:
                        self.log.warning(
                            "Unable to sort files by creation date, ignoring `recent_only=True`"
                        )
                ...
                for file_ind, _file in enumerate(new_file_lists[root][structure_type]):
                    ...
                    if structure_count > 1:
                        self.log.error(
                            "Found double in database, this needs to be dealt with manually: {}".format(
                                _file
                            )
                        )
                    ...
                    if structure_count >= 1:
                        ...
                        if self.recent_only:
                            skipping = len(
                                new_file_lists[root][structure_type][file_ind:]
                            )
                            skipped += skipping
                            for other_file in new_file_lists[root][structure_type][
                                file_ind:
                            ]:
                                _add_to_delete_lists(
                                    other_file, root, new_file_lists, delete_list
                                )
                            break

                        assert (
                            len(new_file_lists[root][structure_type])
                            == len(file_lists[root][structure_type]) + skipped
                        )

            for _type in types:
                new_file_lists[root][_type] = [
                    _file
                    for _file in new_file_lists[root][_type]
            ...
            for file_type in types:
                if (
                    ...
                    raise RuntimeError(
                        "{} count {} does not match number of files {}".format(
                            file_type,
                            len(new_file_lists[root][file_type]),
                            new_file_lists[root][file_type + "_count"],
                        )
    ...
    def _display_import(self):
        ...
        if self.recent_only:
            print(
                "\t\tSkipped {} files filtered by creation date (st_ctime).".format(
                    self.skipped
                )
            )
...
def _add_to_delete_lists(_file, root, new_file_lists, delete_list):
    ...
    for _type in structure_exts:
        ...
        if (
            fname_trial in new_file_lists[root][list_type]
            and fname_trial not in delete_list[list_type]
        ):
