...
def get_equation_of_state(seed, plot=False):
    ...
    if len(volumes) < 3:
        raise RuntimeError(
            "Seed {} does not contain enough energies vs volumes to fit a bulk modulus"
        )
    ...
    for eos_type in types_of_fit:
        ...
        results["summary"].append(
            "fitting parameters = {d[0]:.6f}, {d[1]:.6f}\n".format(d=eos.fit_parameters)
        )
        results["summary"].append("rrrmsd = {:10.10f} %\n".format(eos.rrmsd))
        results["summary"].append(
            "bulk modulus = {d[0]:.6f} +/- {d[1]:.6f} GPa\n".format(
                d=(eos.bulk_modulus, eos.bulk_modulus_err)
            )
        )
...
class EquationOfState:
    ...
    def fit(self, volumes, energies):
        ...
        self.popt, _ = optimize.leastsq(
            self.residual, self.p0, args=(volumes, energies)
        )
        self.rrmsd = np.sqrt(
            np.sum(((energies - self.evaluate(volumes, *self.popt)) / energies) ** 2)
            / (len(energies) - 1)
        )
...
class TaitEOS(EquationOfState):
    ...
    def evaluate(self, V, B, C):
        nu = V / self.V_0
        return self.E_0 + (B * self.V_0 / C) * (
            nu - 1 + (1 / C) * (np.exp(C * (1 - nu)) - 1)
        )
