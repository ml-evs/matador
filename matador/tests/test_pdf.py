...
class PDFCalculatorTest(unittest.TestCase):
    ...
    def test_auto_images_vs_large(self):
        ...
        doc["pdf_auto_images"] = PDF(
            doc, low_mem=True, num_images="auto", rmax=15, dr=0.1, **{"debug": True, "projected": False}
        )
    ...
    def test_identity_overlap(self):
        ...
        doc["lattice_cart"] = abc2cart(doc["lattice_abc"])
        doc["text_id"] = ["pdf", "test"]
        doc["pdf_smear"] = PDF(doc, num_images=3, dr=0.001, gaussian_width=0.1, style="smear", debug=False, low_mem=True)
    ...
    def test_overlap_smear_vs_hist(self):
        ...
        doc["pdf_smear"] = PDF(doc, num_images=3, dr=0.01, gaussian_width=0.1, projected=False, style="smear", low_mem=True)
    ...
    def test_pdf_primitive_vs_supercell(self):
        ...
        test_doc["pdf"] = PDF(test_doc, dr=0.01, low_mem=True, rmax=10, num_images="auto", debug=DEBUG)
        supercell_doc["pdf"] = PDF(supercell_doc, dr=0.01, low_mem=True, rmax=10, num_images="auto", debug=DEBUG)
    ...
    def test_single_atom_pdf(self):
        ...
        doc["text_id"] = ["smear"]
        doc["pdf_smear"] = PDF(
            doc, num_images=num_images, gaussian_width=0.01, dr=0.1, rmax=rmax, lazy=True, style="smear", debug=True
        )
...
class PDFFactoryCalculatorTest(unittest.TestCase):
    ...
    def test_concurrent_pdf(self):
        ...
        print(
            "Corresponding to a speedup of {:.1f} vs ideal {:.1f}".format(serial_elapsed / factory_elapsed, pdf_factory.nprocs)
        )
    ...
    def test_concurrent_pdf_stoichs(self):
        ...
        hull = QueryConvexHull(
            cursor=cursor, no_plot=True, hull_cutoff=0.5, summary=True, elements=["K", "Sn", "P"], quiet=True
        )
